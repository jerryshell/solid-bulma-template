        ...
        <div class="level-left">
          <div>
            <p class="title is-5 is-spaced">
              <a onClick={() => setRoute("OrderUpdate")}>{props.id}</a>
            </p>
            <p class="subtitle is-7">
              <span>2024-10-18 17:38 by </span>
              <a onClick={() => setRoute("CustomerUpdate")}>{props.customer}</a>
            </p>
          </div>
        </div>
        <div class="level-right">
          <div class="has-text-right">
            <p class="title is-5">${props.total}</p>
            <span
...
const MostPopularBooksItem = (props: { name: string; isbn: string; sold: number }) => {
  return (
    <>
      <div class="media">
        ...
        <div class="media-content">
          <p class="title is-5 is-spaced">
            <a onClick={() => setRoute("BookUpdate")}>{props.name}</a>
          </p>
          <p class="subtitle is-7">ISBN: {props.isbn}</p>
        </div>
        <div class="media-right">{props.sold} sold</div>
      </div>
    </>
  );
};

const MostLoyalCustomersItem = (props: { name: string; country: string; orders: number }) => {
  return (
    <>
      <div class="media">
        <div class="media-content">
          <p class="title is-5 is-spaced">
            <a onClick={() => setRoute("CustomerUpdate")}>{props.name}</a>
          </p>
          <p class="subtitle is-7">{props.country}</p>
        </div>
        <div class="media-right">{props.orders} orders</div>
      </div>
    </>
  );
};
    ...
    <>
      <div class="card">
        <div class="card-content">
          ...
          <a class="button is-link is-outlined" onClick={() => setRoute("Orders")}>
            View all orders
          </a>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-content">
          ...
          <For each={mostPopularBooks}>
            {(item) => (
              <>
                <MostPopularBooksItem name={item.name} isbn={item.isbn} sold={item.sold} />
              </>
            )}
          </For>
          <a class="button is-link is-outlined" onClick={() => setRoute("Books")}>
            View all books
          </a>
        </div>
      </div>
    ...
    <>
      <div class="card">
        <div class="card-content">
          ...
          <a class="button is-link is-outlined" onClick={() => setRoute("Customers")}>
            View all customers
          </a>
        </div>
      </div>
